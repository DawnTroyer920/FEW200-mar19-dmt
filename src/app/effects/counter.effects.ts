...
import { tap, count, map, filter } from 'rxjs/operators';
...
export class CounterEffects {
  ...
  loadCountBy$ = this.actions$
    .pipe(
      ofType(appActions.APP_START),  // if this is the app start
      ...
      map(by => new countActions.SetCountBy(by)) // displatch this intor the reducer
    );

  @Effect({ dispatch: false }) saveCountBy$ = this.actions$
    .pipe(ofType(countActions.COUNT_BY_SET),  // Action {type: string}
  ...
  // @Effect({ dispatch: false }) logAllActions$ = this.actions$
  //  .pipe(
